...
class UnpackServer():
    ...
    def __init__(self):
        # super().__init__()
        # похоже это от наследования осталось, здесь ненужно
        self.cache_folder = Path('./nastavnik')
        self.storage_folder = Path('./storage')

    def _normalize_filepath(self, *args, extension: str = None) -> Path:
        '''Метод создает Path до нового файла в папке cache_folder
           из переданных аргументов и extension'''
        if len(args) < 1:
        ...
        # return p
        # Я не самым красивым образом переписал,
        # но не хочу сильно переделывать реализацию
        if extension:
            args = list(args)
            # делаю последний элемент строкой, обрезаю последнюю точку
            # и добавляю расширение
    ...
    def get_common_file(self, params):
        ...
        # добавляю code в общий список, здесь не проработано поведение,
        # если у меня будет name как код, то я перетру один файл вторым
        ...
        # 2. Логирование уровня error пишется в инфо,
        #    нет информации об именах файлов
        ...
        for file in files:
            ...
            # if 'type' in file:
            #    filepath = self._normalize_filepath(file['name'],
            #                                        extension=file['type'])
            if 'name' in file:
                filepath = self._normalize_filepath(
                    file['name'],
                    extension=file['type'] if 'type' in file else None)
            else:
                filepath = self.cache_folder / file['code']
            # if not filepath.exists():
            #   logger.info('File path not exist. Get file from storage.')
            # я здесь в начале купился на то, что вроде бы проверяется файл,
            # но в реальности нет транзакционной целостности,
            # и этот код - ошибка. Поменял сообщение на стандарт
            try:
                shutil.copyfile(storage_filepath, filepath)
            except FileNotFoundError:  # замена проверке
                logger.info(f"Error: Source file '{storage_filepath}' not"
                            f"found, or destination directory does"
                            f"not exist.")
        ...
        # похоже недовыпилили из живого кода,
        # работать не будет, т.к. не определен normalize_path
        # if 'path' in params:
...
answer = v.get_common_file({'code': 'main', 'file': [
                         {'code': '3456dcgbyt',
                          'name': '45.00.679.0.', 'type': '.svg'}]})
